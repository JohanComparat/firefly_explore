...
def plot_az_boss(imf_ref):
	...
	p.grid()
	p.text(-2.1, 75.,'eBOSS '+imf_ref.split('_')[0] +' '+imf_ref.split('_')[1] )
	p.tight_layout()
	...
	p.grid()
	p.tight_layout()
	p.savefig(os.path.join(out_dir, "no_Y_age_metallicity_"+imf_ref+"eboss_04.png" ))
	p.clf()
	...
	p.grid()
	p.tight_layout()
	p.savefig(os.path.join(out_dir, "no_X_age_metallicity_"+imf_ref+"eboss_04.png" ))
	p.clf()
	...
	p.legend(loc=0, frameon = False)
	p.grid()
	p.text(-2.1, 75.,'eBOSS '+imf_ref.split('_')[0] +' '+imf_ref.split('_')[1] .split('_')[0] +' '+imf_ref.split('_')[1] )
	p.tight_layout()
	p.savefig(os.path.join(out_dir, "no_XY_age_metallicity_"+imf_ref+"eboss_04.png" ))
	p.clf()
