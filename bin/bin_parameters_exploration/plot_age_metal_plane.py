...
def plot_az_boss(imf_ref):
	...
	p.grid()
	p.text(-2.1, 7.5,'eBOSS '+imf_ref.split('_')[1] )
	p.tight_layout()
	...
	p.legend(loc=0, frameon = False)
	p.grid()
	p.text(-2.1, 7.5,'eBOSS '+imf_ref.split('_')[1] .split('_')[0]  )
	p.tight_layout()
...
def plot_az_sdss(imf_ref):
	...
	p.colorbar(shrink=0.7, label=r'$\log_{10}(N)$')
	#p.legend(loc=0, frameon = False)
	p.text(-2.1, 7.5,'SDSS '+imf_ref.split('_')[1] .split('_')[0]  )
	p.grid()
	p.tight_layout()
...
def plot_az_deep2(imf_ref):
	...
	p.colorbar(shrink=0.7, label=r'$\log_{10}(N)$')
	p.legend(loc=0, frameon = False)
	p.text(-2.1, 7.5,'DEEP2 '+imf_ref.split('_')[1] .split('_')[0]  )
	p.grid()
	p.tight_layout()
