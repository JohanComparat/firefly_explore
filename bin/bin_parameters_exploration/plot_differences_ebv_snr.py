...
m_bins = n.arange(-.7, .7, 0.05)


def plotDIFF_vs_z_fixed_imf_deep2(imf_ref, imf_1, imf_2):
...
z_flg = 'ZQUALITY'
z_name = 'ZBEST'
snr_name = 'SNR_MEDIAN_ALL'
deep2_zOk = (deep2[z_name] > 0.6) & (deep2[z_flg]>=2.) & (deep2[z_name] < 1.2) & (deep2['SSR']>0) & (deep2['TSR']>0) & (deep2['SSR']<=1.0001) & (deep2['TSR']<=1.0001)
deep2_sel_04 = (deep2_zOk) & (deep2[stellar_mass] < 10**14. ) & (deep2[stellar_mass] > 0. )  & (deep2[stellar_mass] >= deep2[stellar_mass+'_low_1sig'] ) & (deep2[stellar_mass] <= deep2[stellar_mass+'_up_1sig'] ) & ( - n.log10(deep2[stellar_mass+'_low_1sig'])  + n.log10(deep2[stellar_mass+'_up_1sig']) < 0.8 )&(deep2[ebv]>0)&(deep2[snr]>0)
# defines ref quantity
snr = deep2[snr_name][deep2_sel_04]
...
# figure
p.figure(2, (4.5, 4.5))
p.axes([0.18,0.18,0.73,0.73])
p.hist2d(snr, delta_m1, bins=[n.arange(0,20,0.5),m_bins],  cmin=1, cmax=100)
cb=p.colorbar(shrink=0.8)
cb.set_label('Number of galaxies')
#p.plot(snr, delta_m2, rasterized=True)
#p.plot(m_bins, norm.pdf(m_bins, loc=0, scale=1), label='N(0,1)', ls='dashed')
p.xlabel('median SNR')
p.ylabel(r'$E(B-V)-E(B-V)_{ref}$')
p.title('DEEP2 '+imf_ref.split('_')[0])
p.legend(loc=2, frameon = False, fontsize=11)
#p.xlim((-0.7, 0.7))
p.grid()
p.savefig(os.path.join(out_dir, "delta_EBV_SNR_deep2"+imf_ref.split('_')[0]+".png" ))
p.clf()



plotDIFF_vs_z_fixed_imf_deep2(	imf_ref = imfs[0], 	imf_1   = imfs[1], 	imf_2   = imfs[2])
plotDIFF_vs_z_fixed_imf_deep2(	imf_ref = imfs[3], 	imf_1   = imfs[4], 	imf_2   = imfs[5])
plotDIFF_vs_z_fixed_imf_deep2(	imf_ref = imfs[6], 	imf_1   = imfs[7], 	imf_2   = imfs[8])
