...
def plot_dm_boss(imf_ref):
	...
	p.xlim((-0.01,0.71))
	p.ylim((5.5,12.5))
	p.text(0.01, 6.,'eBOSS '+imf_ref.split('_')[0] +' '+imf_ref.split('_')[1] )
	p.tight_layout()
	p.grid()
...
def plot_dm_sdss(imf_ref):
	...
	p.xlim((-0.01,0.71))
	p.ylim((5.5,12.5))
	p.text(0.01, 6.,'SDSS '+imf_ref.split('_')[0] +' '+imf_ref.split('_')[1] )
	p.grid()
...
def plot_dm_deep2(imf_ref):
	...
	p.xlim((-0.01,0.71))
	p.ylim((5.5,12.5))
	p.text(0.01, 6.,'DEEP2 '+imf_ref.split('_')[0] +' '+imf_ref.split('_')[1] )
	#p.legend(loc=0, frameon = False)
	p.grid()
