...
def plotDIFF(imf_ref, imf_1, imf_2, m_bins = n.arange(-10., 10., 0.1)):
	...
	p.hist(delta_m3[delta_m3>0], bins=m_bins, histtype='step', label=imf_2.split('_')[1]+"-"+imf_1.split('_')[1] , normed=True )
...
def plotDIFF(imf_ref, imf_1, imf_2, m_bins = n.arange(-10., 10., 0.1)):
	...
	p.xlim((-5.5, 5.5))
	p.grid()
	p.savefig(os.path.join(out_dir, "delta_M_distribution_sdss"+imf_ref.split('_')[1]+".png" ))
	p.clf()
	# normalized comparison ratio
	delta_m1 = (A_04_1-A_04_ref)*(eA_04_ref**2.+eA_04_1**2.)**(-0.5)
	delta_m2 = (A_04_2-A_04_ref)*(eA_04_ref**2.+eA_04_2**2.)**(-0.5)
	...
	p.xlim((-5.5, 5.5))
	p.grid()
	p.savefig(os.path.join(out_dir, "delta_A_distribution_sdss"+imf_ref.split('_')[1]+".png" ))
	p.clf()
	# normalized comparison ratio
	delta_m1 = (Z_04_1-Z_04_ref)*(eZ_04_ref**2.+eZ_04_1**2.)**(-0.5)
	delta_m2 = (Z_04_2-Z_04_ref)*(eZ_04_ref**2.+eZ_04_2**2.)**(-0.5)
	...
	p.xlim((-5.5, 5.5))
	p.grid()
	p.savefig(os.path.join(out_dir, "delta_Z_distribution_sdss"+imf_ref.split('_')[1]+".png" ))
	p.clf()
